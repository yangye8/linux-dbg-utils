...
{
	printf("SCCB Tool\n");
	printf("sccb -[w:r] <i2c_adapter> <i2c_addr> <reg_size>[w:s:b] <off> <val>\n\n");
}
...
static int write_sccb_register(uint32_t register_address,
			       uint8_t register_value, enum reg_size data_width)
{
	int ret = 0;
...
static int read_sccb_register(uint32_t register_address,
			      uint8_t * register_value,
			      enum reg_size data_width)
{
	int ret = 0;
	uint8_t val;
